...
import { buildSandboxCallbacks } from './plugin-sandbox-callbacks';
...
export class PluginLoaderService implements OnModuleInit, OnModuleDestroy {
  ...
  private async enableSandboxed(pluginId: string, plugin: PluginInstance): Promise<void> {
    ...
    const { onHookSubscribe, onWebhookSubscribe, onLog, onSearchProviderRegister, onWorkerExit } =
      buildSandboxCallbacks({
        pluginId,
        plugin,
        context,
        logger: this.logger,
        ...
        getSearchRegistry: () => this.getSearchRegistry(),
        searchProviderMode: () => this.configService.get<string>('search.provider', 'auto'),
      });
