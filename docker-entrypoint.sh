...
# openwa-lab user via gosu so the Node process never holds root privileges.
set -e
...
# Chromium resolves its home from the passwd entry (no /home/openwa-lab exists), so it hard-crashes at
# launch unless its config/cache dirs exist and are writable. XDG_CONFIG_HOME/XDG_CACHE_HOME (set in
# the image) point here; create them owned by openwa-lab. On a read_only rootfs these live on tmpfs /tmp,
...
chown openwa-lab:openwa-lab "${XDG_CONFIG_HOME:-/tmp/.config}" "${XDG_CACHE_HOME:-/tmp/.cache}"
