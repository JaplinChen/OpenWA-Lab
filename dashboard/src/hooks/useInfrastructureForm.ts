...
import { useInfraRestart, useInfraBackup } from './useInfraMaintenance';
import type {
  DatabaseConfig,
  RedisConfig,
  StorageConfig,
...
export function useInfrastructureForm() {
  ...
  const { showRestartModal, setShowRestartModal, restartCountdown, restartStatus, handleRestart } =
    useInfraRestart(pendingProfiles, previousProfiles);
  const { migrating, handleExportBackup, handleImportBackup } = useInfraBackup();
